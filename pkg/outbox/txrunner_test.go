...
func TestNewTxRunner_propagatesFnError(t *testing.T) {
	...
	got := r.RunInTx(context.Background(), func(tx outbox.Tx) error {
		return sentinel
	})
...
func TestTxRunner_isInterfaceSatisfiedByConstructor(t *testing.T) {
	// Compile-time check that the constructor returns an outbox.TxRunner.
	var _ outbox.TxRunner = outbox.NewTxRunner(nil)
}
